...
# также поступаем в других модулях
...
def add_runoun2(word,wordmn,r,o,skl=None,sense=None):
	'''определяет склонение (если не указано) и добавляет слово в словарь (перезаписывает)'''
	if skl==None:
...
def ____Adj():
	...
	add_ruadj('его' ,'его' ,adj_std_ends_NONE)
	add_ruadj('её' ,'её' ,adj_std_ends_NONE)

	adj_std_ends_yejai={# и
		'm' :('й','его','ему',('его','й'),'им' ,'ём',None ),
		's' :('ё','его','ему',('ё',  'ё'),'им' ,'ём',None),
		'g' :('я','ей' ,'ей' ,('ю',  'ю'),'ей' ,'ей',None),
		'mn':('и','их' ,'им' ,('их', 'и'),'ими','их',None),
	}
	add_ruadj('мой' ,'мо' ,adj_std_ends_yejai)
	add_ruadj('твой','тво' ,adj_std_ends_yejai)

	adj_std_ends_ooai_N={# и
		'm' :('ой','ого','ому',('ого','ой'),'им' ,'ом',None ),
		's' :('ое','ого','ому',('ое', 'ое'),'им' ,'ом',None),
	...
	add_ruadj('летучий','летуч' ,adj_std_ends_ieai)
	add_ruadj('хороший','хорош' ,adj_std_ends_ieai)

	adj_std_ends_iejai={# и
		'm' :('ий','его','ему',('его','ий'),'им' ,'ем','' ),
		's' :('ее','его','ему',('ее', 'ее'),'им' ,'ем','е'),
		'g' :('яя','ей' ,'ей' ,('юю', 'юю'),'ей' ,'ей','я'),
		'mn':('ие','их' ,'им' ,('их', 'ие'),'ими','их','и'),
	}
	add_ruadj('синий','син' ,adj_std_ends_iejai)

	adj_std_ends_k_yoay={# ы
		'm' :('кий','кого','кому',('кого','кий'),'ким' ,'ком','ок' ),
		's' :('кое','кого','кому',('кое', 'кое'),'ким' ,'ком','ко'),
		'g' :('кая','кой' ,'кой' ,('кую', 'кую'),'кой' ,'кой','ка'),
	...
	add_ruadj('добрый' ,'добр' ,adj_std_ends_yoay)
	...
	add_ruadj('некоторый' ,'некотор' ,adj_std_ends_yoay_N)

	adj_std_ends_n_yoay={# ы
	...
	add_ruadj('определённый' ,'определён' ,adj_std_ends_n_yoay)

	adj_std_ends_n_yoay2={# ы
	...
	add_ruadj('длинный' ,'длин' ,adj_std_ends_n_yoay2)
	add_ruadj('чёрный' ,'чёр' ,adj_std_ends_n_yoay2)# черна черно черны
	add_ruadj('красный' ,'крас' ,adj_std_ends_n_yoay2)
...
def ____Num():
	...
	def add_runum(name,quantity,chis,start,ends,r='m',show_name=None):
		if show_name==None: show_name = name
		show_num_map[show_name]=lambda st: show_num1(st,start ,ends)
		ruwords[name]=     StNum(show_name,quantity  ,False,r,chis,'ip')
	...
	add_runum('один','1','ed','од' , one_skl)
	add_runum('одно','1','ed','од' , one_skl,'s','один')
	add_runum('одна','1','ed','од' , one_skl,'g','один')
	#add_runum('одни','1','ed','од' , one_skl,'s')

	show_num_map['два']= (lambda st: 
	...
	ruwords['две']=    StNum('два'   ,'2-4',False,'g','mn','ip')
...
def ____Verb():
	...
	add_ruverb('смотреть',"посмотреть"   ,'nesov','смотр',
			{    'neopr':"еть",
				 'povel':("и","ите"),
				 'nast': ("ю","ишь","ит","ит","ит","им","ите","ят")
			}   )
	add_ruverb('посмотреть',"смотреть"   ,'sov','посмотр',
			{    'neopr':"еть",
				 'povel':("и","ите"),
				 'nast': ("ю","ишь","ит","ит","ит","им","ите","ят")
			}   )

	add_ruverb('брать',"взять"   ,'nesov','б',
			{    'neopr':"рать",
				 'povel':("ери","ерите"),
				 'nast': ("еру","ерёшь","ерёт","ерёт","ерёт","ерём","ерёте","ерут")
			}   )
	add_ruverb('взять',"брать"   ,'sov','в',
			{    'neopr':"зять",
				 'povel':("озьми","озьмите"),
				 'nast': ("озьму","озьмёшь","озьмёт","озьмёт","озьмёт","озьмём","озьмёте","озьмут")
			}   )
	...
	# ????
	add_ruverb('стать','есть (быть)'   ,'sov','ста',
			{    'neopr':"ть",
				 'povel':("нь","ньте"),
				 'nast': ("л","л","л","ло","ла","ли","ли","ли")
			}   )
